	...
	app.models.Client.validate_consumerKey(req.param('consumerKey'), function(err, client){
		if(err){
			...
			if(client){
				...
				if(req.param('limit'))
					limit = req.param('limit');
				app.models.Twitts.find({}, null, { limit: limit, desc: 'created_at'}, function(err, twitts){
					if(err){
				...
				app.models.Tag.tag_by_value(req.param('value'), function(err, tag){
					if(err){
						...
						if(req.param('limit'))
							limit = req.param('limit');
						console.log(tag._id);
						app.models.Twitts.find({ 'tag_id': tag._id }, null, { limit: limit, desc: 'created_at'}, function(err, twitts){
							if(err){
								res.json({ type: 'error', value: 'No se han posdido listar los twitts.' });
							}else{
								res.json({ type: 'success', data: twitts });
